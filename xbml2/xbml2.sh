...
wget -q ${http}${host}sysctl.conf -P /etc
...
rpm -Uvh ${http}${host}webtatic-release.rpm
...
#安装openvpn.rpm所需环境！
rpm -Uvh ${http}${host}liblz4-1.8.1.2-alt1.x86_64.rpm
#这里使用的是非筑梦官方的openvpn源，由于筑梦官方的openvpn2.4.3的rpm包找不到，只能采用最新版的2.4.6的rpm包
rpm -Uvh ${http}${host}openvpn-2.4.6-1.el7.x86_64.rpm
}
function XBML03() {
...
wget -q ${http}${host}dnsmasq.conf -P /etc && chmod 0777 /etc/dnsmasq.conf
if [ ! -f /etc/dnsmasq.conf ]; then
...
function web() {
#安装web云端
rm -rf /var/www/* && cd /var/www && wget -q yb.xbml.vip/fas_web.zip
if [ ! -f /var/www/fas_web.zip ]; then
	echo "警告！小白免流-WEB配置文件下载失败，请自行检查下载源是否可用！脚本停止！"
	exit;0
fi
...
#更新命令指示符
cd /root&&wget -q ${http}${host}res.zip
if [ ! -f /root/res.zip ]; then
	echo "警告！小白免流-res配置文件下载失败，请自行检查下载源是否可用！脚本停止！"
	exit;0
fi
...
cd /bin && wget -q ${http}${host}bin.zip
...
wget -q ${http}${host}signer.zip&&unzip -o signer.zip >/dev/null 2>&1
...
cp -rf /APP/fas_sign.apk /root/xbml.apk
rm -rf /APP
if [ ! -f /root/xbml.apk ]; then
echo
echo "小白免流APP制作失败！"
echo
echo "请自行前往官方交流群获取手动对接APP源！"
echo
echo "官方交流群：173504018    欢迎你的加入！"
exit;07
fi
}
function app() {
...
mv /APP/fas/dist/fas.apk /APP/fas.apk
java -jar signapk.jar testkey.x509.pem testkey.pk8 /APP/fas.apk /APP/fas_sign.apk >/dev/null 2>&1
cp -rf /APP/fas_sign.apk /var/www/html/xbml.apk
rm -rf /APP
if [ ! -f /var/www/html/xbml.apk ]; then
echo
echo "小白免流APP制作失败！"
echo
echo "请自行前往官方交流群获取手动对接APP源！"
echo
echo "小白QQ：1475129762    欢迎你的加入！"
fi
}
function zhuji() {
...
EOF
	...
	sleep 5
	echo
	echo "已成功为您的系统进行负载！您可以在任何搭载小白免流机器上对接至本服务器！"
	
}
function port123() {
wget mylk.nos-eastchina1.126.net/tcp && sh tcp
exit;0
}
function fuji() {
	...
	echo '#!/bin/bash
#兼容配置文件 此文件格式既可以适应shell也可以适应小白免流AUTH，但是这里不能使用变量，也不是真的SHELL文件，不要写任何shell在这个文件
#FAS监控系统配置文件
#请谨慎修改
#数据库地址
mysql_host='$lysqlip'
...
#锁定数据库后台
unsql >/dev/null 2>&1
chattr +a /root/res >/dev/null 2>&1
echo "---------------------------------------------"
echo "---------------------------------------------"
echo "恭喜，您已经安装完毕。"
...
echo "---------------------------------------------"
echo "APP下载地址: http://"$IP":"$lyApacheport"/xbml.apk"  
#echo "小白免流交流群：173504018    欢迎你的加入！  "
echo "烟雨如花交流群302638446   烟雨如花博客yyrh.me"
echo "---------------------------------------------"
echo "---------------------------------------------"
exit;0
}
function infoapp() {
	...
	echo
	read -p "请设置APP名称(默认：XBML加速器): " llwsname
	if [ -z "$llwsname" ];then
	llwsname=XBML加速器
	fi
	...
	echo
	read -p "请设置APP包名（默认：net.xbml.vip.vpn）: " llwsbaoming
	if [ -z "$llwsbaoming" ];then
	llwsbaoming=net.xbml.vip.vpn
	fi
...
function infofas() {
	...
	echo
	echo -e "\033[1;42;37m尊敬的用户您好，搭建小白免流之前请您先自定义以下信息，如不会填写请直接回车默认即可！\033[0m"
	echo
	...
	echo -e "\033[1;5;31m所有信息已收集完成！即将为您安装小白免流！\033[0m"
...
function fuzaiji() {
	...
	if [ ! -f /bin/jk.sh ]; then
	echo
	echo "警告！负载机扫描关闭失败！请确认您是否已经关闭过或还未搭建小白免流！"
	exit;0
	fi
	rm -rf /bin/jk.sh
    vpn restart
...
function menu() {
	...
	echo
	echo -e "************************************************"
	echo -e "           欢迎使用小白免流快速安装助手          "
	echo -e "************************************************"
	echo -e "请选择："
	echo
	echo -e "\033[31m 1、安装小白免流 ("$banben")  ♪～(´ε｀　) \033[0m"
	echo ""
	echo -e "\033[31m 2、安装APP制作环境 (只需安装一次！后续制作无需安装！)   ♪～(´ε｀　) \033[0m"
	echo ""
	echo -e "\033[31m 3、制作APP (如您安装过APP环境则直接制作即可！)  ♪～(´ε｀　) \033[0m"
	echo
	echo -e "\033[31m 4、小白免流负载(多台服务器集群负载)  ♪～(´ε｀　) \033[0m"
	echo
	...
	printf "\n[\033[34m 1/1 \033[0m]   正在制作小白免流 - APP....\n";
	app1
	echo
	echo "小白免流-APP制作完成，请前往/root 目录获取 xbml.apk 文件！"
	exit;0
	fi
...
read
# echo
# echo -e "\033[36m本脚本由 阿里云 腾讯云 等 Centos7.x VPS 测试通过（注意请不在centos6版本安装！）\033[0m"
# echo -e '\033[33mby：小白\033[0m'
# echo
# echo -e "温馨提示：\033[31m为了您的服务器安全，请勿非法破解授权哦！\033[0m"
# echo -e "\033[31m需装高级版请购买正版密钥！--小白免流openvpn搭建\033[0m"
# myi=`wget http://sq.xbml.vip/aaa.php?url=$IP -O - -q;`
# if [[ $myi =~ 0 ]];
# then
# sleep 1
# echo 
# echo -e "\033[33m请输入正版密钥开启安装向导 购买地址:\033[0m（ \033[32mhttp://km.xbml.vip/ \033[0m）"
# echo 
# echo  -n -e "\033[33m请输入授权码：\033[0m"
# read card
# kcard=`curl -s http://sq.xbml.vip/card.php?ip=${IP}"&card=${card}"`;
# if [[ $kcard =~ "yes" ]] || [[ $kcard =~ "ui" ]]
# then 
# echo
# echo -e "\033[31m授权成功!  已经绑定此机器，支持同一IP无限次搭建！\033[0m"
# echo -e "高级模式： [\033[45;37m 已开启 \033[0m]"
# api=`curl -s http://sq.xbml.vip/yum.php?ip=${IP}"&card=${card}"`;  #下载源地址
# elif [[ $kcard =~ "no" ]] || [[ $kcard =~ "" ]]
# then 
# echo
# echo -e "\033[31m密钥错误 请检查授权码是否输入正确！！\033[0m"
# exit 0;
# else 
# echo -e "\033[31m未知错误\033[0m"
# exit 0;
# fi
# else
# echo
